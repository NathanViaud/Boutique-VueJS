<template>
    <div class="card">
        <h3 class="mb-10">{{ product ? 'Editer un produit ' : 'Créer un produit' }}</h3>
        <form @submit="trySubmit">
            ...
            <div class="d-flex flex-column mb-20">
                ...
                <textarea v-model="(description.value.value as string)"/>
...
const validationSchema = toFormValidator(
    z.object({
        title: z.string(required).min(1, { message: 'Le titre doit faire au moins 1 caractère' }).max(20, { message: 'Le titre doit faire moins de 10 caractère' }),
        image: z.string(required),
        price: z.number(required).min(0, { message: 'Le prix doit être supérieur à 0€' }).max(15000, { message: 'Le prix doit être inférieur à 15000€' }),
